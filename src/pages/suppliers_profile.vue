          ...
          <div class="bg-white shadow-xl rounded-lg py-3">
            <div class="photo-wrapper p-2">
              <img 
                ...
                :src="getProfileImageUrl(supplier.user_img)" 
...
export default {
  ...
  methods: {
    ...
    getProfileImageUrl(imageFileName) {
      ...
      if (imageFileName) {
        ...
        if (imageFileName.startsWith('http')) {
          ...
          return `${this.apiBaseUrl}/api/user/profile-image/${imageFileName}`;
        }
      }
      
      // Default fallback
      return `${this.apiBaseUrl}/api/user/profile-image/dummy_profile.png`;
    },
    handleImageError(e) {
      console.error('Image failed to load:', e.target.src);
      e.target.src = `${this.apiBaseUrl}/api/user/profile-image/dummy_profile.png`;
    },
    async fetchSuppliers() {
      try {
        ...
        if (response.data && response.data.status === 'success') {
          this.allSuppliers = response.data.data.map(supplier => ({
            ...supplier,
